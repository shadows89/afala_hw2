...
char* policies[] = {"other","fifo","rr","BUG","mq"};
char* reasons[] = {"0","TASK_CREATED","TASK_ENDED","TASK_YIELD","MQ_BECAME_OVERDUE","TASK_ENTERED_SLEEP","TASK_WOKEN","FINISHED_TIMESLICE"};
...
int main(int argc, char** argv){
	...
	for (i=0;i<len ;i++)
		printf("%04d     %04d    %8s   %8s       %d %s\n",info[i].previous_pid, info[i].next_pid,
			...
			info[i].time,reasons[info[i].reason]);
	return 0;
}
