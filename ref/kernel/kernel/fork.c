	    ...
	    struct pt_regs *regs, unsigned long stack_size)
{
	...
	*p = *current;
	...
	if (!current->time_slice)
		BUG();
	if (current->policy != SCHED_MQ || current->mq_trials==0 ) {
		p->time_slice = (current->time_slice + 1) >> 1;
		p->first_time_slice = 1;
		current->time_slice >>= 1;
	}
	p->sleep_timestamp = jiffies;
	p->mq_trials=current->mq_trials;
	if (current->policy==SCHED_MQ&&current->mq_trials>0) {
		p->time_slice = current->time_slice;
		p->first_time_slice = 1;
		p->mq_first_trial_num=current->mq_trials;
	}
	if (!current->time_slice) {
		/*
	...
	if (clone_flags & CLONE_VFORK)
		 ...
		 */
		if(p->policy==SCHED_MQ&&p->mq_trials>0){
			wake_up_father(current);
		}
		if (p->policy!=SCHED_MQ||p->mq_trials>0) {
			current->need_resched=1;
			if (current->reason>TASK_CREATED||current->reason==0){
